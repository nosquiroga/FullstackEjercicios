...
Promise.all([readJSON(filmsFile), readJSON(planetsFile)])
  .then(([films, planets]) => {
    filmPlanets = films.find(film => film.id === "5");
    planetsFilm = planets
      .filter(planet => filmPlanets.planets.includes(planet.id))
      .map(planet => planet.name);
    return writeJSON(ej5_4File, planetsFilm);
  })
  .then(() => {
    console.log("Se guardaron los planetas con Promise.all()!");
  })
  .catch(err => {
    console.log(err);
  });
